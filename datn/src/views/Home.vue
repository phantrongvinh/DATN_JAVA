...
const audiencesView = computed(() => {
  return audiences.value.slice(0, 3).map((element, index) => ({
    ...element,
    image: images[index],
  }))
})
      ...
      >
        ...
        <div class="absolute bottom-6 left-6 right-6 text-ivory">
          ...
          <h3 class="font-display text-2xl">
            Dành cho {{ item.name === 'Men' ? 'Nam' : item.name === 'Women' ? 'Nữ' : 'Trẻ em' }}
          </h3>
          ...
          >
            <span class="mt-2 inline-flex items-center gap-1 text-sm cursor-pointer">
              Khám phá
              <ArrowRight class="h-4 w-4 transition-transform group-hover:translate-x-1" />
            </span>
          </RouterLink>
