...
public class ProductSpecification {
    ...
    public static Specification<Product> adminFilter(AdminProductFilterDTO filterDTO) {
    return (root, query, cb) -> {
        List<Predicate> predicates = new ArrayList<>();

        // Admin thấy cả sản phẩm đã xóa mềm -> không filter deletedAt

        if (filterDTO.getAudienceId() != null) {
            predicates.add(cb.equal(root.get("targetAudience").get("id"), filterDTO.getAudienceId()));
        }

        if (filterDTO.getBrandId() != null) {
            predicates.add(cb.equal(root.get("brand").get("id"), filterDTO.getBrandId()));
        }
